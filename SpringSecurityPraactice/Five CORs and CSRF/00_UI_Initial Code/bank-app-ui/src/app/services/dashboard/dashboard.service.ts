...
export class DashboardService {
  ...
  saveMessage(contact: Contact) {
    return this.http.post(
      ...
      contact,
      //{ observe: "response", withCredentials: true } by doing this we send back
      //all the authentication cookeys to the backend
      //done for protected APIs that nedd to be authenticated
      { observe: "response", withCredentials: true }
    );
  }
}
